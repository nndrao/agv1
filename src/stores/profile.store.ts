...
import { ColumnCustomization, serializeColumnCustomizations, deserializeColumnCustomizations } from '@/components/datatable/stores/columnSerializer';
...
// Factory function to create instance-specific profile stores
export const createInstanceProfileStore = (instanceId: string) => {
  console.log(`[ProfileStore] Creating instance-specific store for: ${instanceId}`);
  
  return create<ProfileStore>()(
    persist(
      (set, get) => ({
      ...
      createProfile: (name, description) => {
        ...
        // Force persistence by accessing the store
        // Note: Each instance will handle its own persistence
      ...
      performDeferredMigration: async () => {
        ...
        try {
          ...
          const storageKey = `grid-profile-storage-${instanceId}`;
    ...
    {
      name: `grid-profile-storage-${instanceId}`,
      ...
      onRehydrateStorage: () => {
        return (state, error) => {
          ...
          const stored = localStorage.getItem(`grid-profile-storage-${instanceId}`);
          if (stored) {
            try {
...
// The ProfileStore type is already defined above in the interface
// Just re-export it
export type { ProfileStore };

// Remove the old convenience hooks as they won't work with instance stores
// Users should use the ProfileStoreProvider and useInstanceProfile hooks instead

// Note: Hydration and migration logic should be handled per instance
// through the ProfileStoreProvider component