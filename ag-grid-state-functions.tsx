...
// Define SortModel type locally since it's not exported in v33+
...
export interface AgGridState {
  ...
  focusedCell?: {
    rowIndex: number;
    column: string;
    rowPinned?: RowPinnedType | null;
  };
  cellRanges?: Array<{
    startRow: { rowIndex: number; rowPinned?: RowPinnedType | null };
    endRow: { rowIndex: number; rowPinned?: RowPinnedType | null };
    columns: string[];
  }>;
...
export function useAgGridStateManager(gridApi: GridApi | null) {
  ...
  const extractState = useCallback((
    ...
    try {
      ...
      // Column groups state - use proper API
      ...
      try {
        ...
        if (allColumns) {
          ...
          allColumns.forEach((col: Column) => {
            ...
            if (parent && parent.getGroupId && !processedGroups.has(parent.getGroupId())) {
              processedGroups.add(parent.getGroupId());
              columnGroupState.push({
                groupId: parent.getGroupId(),
                open: parent.isExpanded()
              });
            }
          });
        }
      ...
      // Extract sort model from column state since getSortModel doesn't exist
      ...
      try {
        // In v33+, sidebar state is managed differently
        const sideBarDef = gridApi.getGridOption('sideBar') as SideBarDef;
        if (sideBarDef) {
          sideBarState = {
            position: (sideBarDef as any).position || 'right',
            visible: true, // Assume visible if sidebar is configured
            openedToolPanel: null // This would need to be tracked separately
          };
        }
      } catch (error) {
  ...
  const applyState = useCallback(async (
    ...
    const defaultOptions: ApplyStateOptions = {
    ...
    try {
      ...
      if (opts.applyColumnGroups && state.columnGroupState) {
        state.columnGroupState.forEach(({ groupId, open }) => {
          try {
            ...
            if (columnGroup && (columnGroup as any).setExpanded) {
              (columnGroup as any).setExpanded(open);
            }
          } catch (error) {
      ...
      if (opts.applySorting && state.sortModel) {
        // Apply sorting through column state since setSortModel doesn't exist in v33+
        const currentColumnState = gridApi.getColumnState();
        const updatedColumnState = currentColumnState.map(col => {
          const sortInfo = state.sortModel.find(s => s.colId === col.colId);
          return {
            ...col,
            sort: sortInfo?.sort || null,
            sortIndex: sortInfo ? state.sortModel.indexOf(sortInfo) : null
          };
        });
      ...
      if (opts.applyRowGrouping && state.rowGroupColumns) {
        ...
        const colsToRemove = Array.from(currentCols).filter(colId => !targetCols.has(colId));
        if (colsToRemove.length > 0) {
          gridApi.removeRowGroupColumns(colsToRemove);
        }

        // Add columns that should be grouped
        const colsToAdd = Array.from(targetCols).filter(colId => !currentCols.has(colId));
        if (colsToAdd.length > 0) {
          gridApi.addRowGroupColumns(colsToAdd);
        }
      }
      ...
      if (opts.applyPivoting) {
        if (state.pivotMode !== undefined) {
          // In AG-Grid v33+, pivot mode is managed through grid options
          gridApi.updateGridOptions({ pivotMode: state.pivotMode });
        }
        ...
        if (state.pivotColumns) {
          ...
          const colsToRemove = Array.from(currentCols).filter(colId => !targetCols.has(colId));
          if (colsToRemove.length > 0) {
            gridApi.removePivotColumns(colsToRemove);
          }

          const colsToAdd = Array.from(targetCols).filter(colId => !currentCols.has(colId));
          if (colsToAdd.length > 0) {
            gridApi.addPivotColumns(colsToAdd);
          }
        }
      }
      ...
      if (opts.applyValueColumns && state.valueColumns) {
        ...
        const colsToRemove = Array.from(currentCols).filter(colId => !targetCols.has(colId));
        if (colsToRemove.length > 0) {
          gridApi.removeValueColumns(colsToRemove);
        }

        const colsToAdd = Array.from(targetCols).filter(colId => !currentCols.has(colId));
        if (colsToAdd.length > 0) {
          gridApi.addValueColumns(colsToAdd);
        }
      }
      ...
      if (opts.applyPagination && state.pagination) {
        if (state.pagination.pageSize !== undefined) {
          // In AG-Grid v33+, pagination page size is managed through grid options
          gridApi.updateGridOptions({ paginationPageSize: state.pagination.pageSize });
        }

        if (state.pagination.paginationPageSizeSelector !== null) {
          // Note: paginationPageSizeSelector application may need to be handled differently in v33+
          // Skipping for now to avoid type errors - this would need specific documentation reference
          console.warn('[AG-Grid State] paginationPageSizeSelector restoration not yet implemented for v33+');
        }
      ...
      if (opts.applyRowPinning) {
        if (state.pinnedTopRowData) {
          gridApi.updateGridOptions({ pinnedTopRowData: state.pinnedTopRowData });
        }
        if (state.pinnedBottomRowData) {
          gridApi.updateGridOptions({ pinnedBottomRowData: state.pinnedBottomRowData });
        }
      }
      ...
      if (opts.applyFocus && state.focusedCell) {
        ...
        gridApi.setFocusedCell(rowIndex, column, rowPinned);
      }
      ...
      if (opts.applySideBar && state.sideBarState) {
        try {
          // In v33+, sidebar management is through grid options
          if (state.sideBarState.visible) {
            gridApi.updateGridOptions({
              sideBar: {
                ...
                position: state.sideBarState.position,
                defaultToolPanel: state.sideBarState.openedToolPanel || 'columns'
              }
            });
          }
        } catch (error) {
